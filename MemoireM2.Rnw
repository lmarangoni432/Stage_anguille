...
total_rive_droite <- round(sum(grdata$effectif_total[grdata$DC == 104], na.rm = TRUE))
total_rive_gauche <- round(sum(grdata$effectif_total[grdata$DC == 102], na.rm = TRUE))
...
graph_annee<- ggplot(annee2) +
  ...
  geom_text(data = totals, aes(x = annee, y = total_effectif, label = format(total_effectif,big.mark=" ")),
...
A noté que pour cette année en rive gauche, \Sexpr{total_rive_gauche} anguilles ont été piégées, soit plus du double des effectifs de l’année 2022 qui était jusqu’à présent la meilleure année.
