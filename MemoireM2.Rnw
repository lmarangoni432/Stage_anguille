...
# Créer une fonction pour formater les nombres avec un espace, sauf pour 2024
format_number <- function(x) {
  if (x == 2024) {
    return(as.character(x))  # Ne pas formater 2024
  } else if (nchar(as.character(x)) == 4) {
    return(as.character(x))  # Ne pas formater les autres années à 4 chiffres
  } else {
    return(format(x, big.mark = " ", scientific = FALSE))
  }
}

# Appliquer cette fonction à tout le document pour afficher les résultats avec des espaces
knit_hooks$set(inline = function(x) {
  if (is.numeric(x)) format_number(x) else x
})
@
...
filecolor=blue, 
urlcolor=blue, 
pdftitle= {Suivi et estimation du recrutement en anguille européenne sur le fleuve Seine\\ Année \Sexpr{annee} }, pdfauthor={Marangoni Lucas},
pdfsubject={stacomi},
...
\fancyhf{}
\rhead{\rightmark}
\lfoot{\footnotesize{Suivi et estimation du recrutement en anguille européenne sur le fleuve Seine, année \Sexpr{annee}}} %Insere un pied de page avec le texte a gauche.
\rfoot{\thepage}
...
###### connexion à la base Stacomi, schéma Seinormigr sur le raspberry ###########
...
##### création de bilan  multiple (plusieurs DC et/ou plusieurs esp mais 1 année) ########

r_mig_mult=new("report_mig_mult")
r_mig_mult=choice_c(r_mig_mult,
                    dc=c(102,104),
                    taxa=c("Anguilla anguilla"),
                    stage=c("4"),datedebut="2024-01-01",datefin="2024-12-31")
...
r_sample_char<-choice_c(r_sample_char,
                        ...
                        taxa=c("Anguilla anguilla"),
                        stage=c("4"),
                        par=c(1786),  #### poids = A111  taille= 1786
                        horodatedebut="2019-01-01",  ### rien avant car pas intégré dans la base
                        horodatefin="2024-12-31",
                        silent=TRUE)
r_sample_char<-connect(r_sample_char)
r_sample_char<-calcule(r_sample_char,silent=TRUE)

data_env<-new("report_env")
data_env<-choice_c(data_env,
                    stationMesure=c("P_coef_maree","Poses_debit","Poses_temp","P_temp_air","Po_niv_aval"),
                    datedebut="2024-01-01",
                    datefin="2024-12-31",
                    silent=FALSE)
data_env<-connect(data_env)
data_env<-data_env@data

r_env<-new("report_env")
r_env<-choice_c(r_env,
                stationMesure=c("Po_niv_aval"),
                datedebut="2019-01-01",
                datefin="2024-12-31",
                silent=FALSE)	
r_env<-connect(r_env)

histo_debit<-new("report_env")
histo_debit<-choice_c(histo_debit,
                stationMesure=c("Poses_debit"),
                datedebut="2008-01-01",
                datefin="2024-12-31",
                silent=FALSE)	
histo_debit<-connect(histo_debit)

histo_debit<-histo_debit@data

hauteur_eau<-r_env@data
grdata<-fun_aggreg_for_plot(r_mig_mult)
annee2<-r_ann@data
bio_ang<-r_sample_char@data
patho <- dbGetQuery(con, "SELECT * FROM seinormigr.tj_pathologieconstatee_pco")
lot <- dbGetQuery(con, "SELECT * FROM seinormigr.t_lot_lot")
ope<-dbGetQuery(con, "SELECT * FROM seinormigr.t_operation_ope")
...
data_histo_RG <- r_mig_interannual@data
...
data_histo_RD <- r_mig_interannual@data

# Calcul des totaux pour la rive droite (102) et la rive gauche (104)
total_rive_droite <- round(sum(grdata$effectif_total[grdata$DC == 102], na.rm = TRUE))
total_rive_gauche <- round(sum(grdata$effectif_total[grdata$DC == 104], na.rm = TRUE))

save(hauteur_eau,grdata,annee2,bio_ang,patho,lot,ope,data_env,histo_debit,total_rive_droite,total_rive_gauche,data_histo_RG,data_histo_RD,file=str_c(datawd,"data_report.Rdata"))

}else{
  load(str_c(datawd,"data_report.Rdata"))
}


# Calculer le nombre total d'anguilles
total_anguilles <- sum(grdata$effectif_total, na.rm = TRUE)

# Calculer le pourcentage pour la rive droite (DC == 104)
effectif_rive_droite <- sum(grdata$effectif_total[grdata$DC == 104], na.rm = TRUE)
pourcentage_rive_droite <- (effectif_rive_droite / total_anguilles) * 100

# Calculer le pourcentage pour la rive gauche (DC == 102)
effectif_rive_gauche <- sum(grdata$effectif_total[grdata$DC == 102], na.rm = TRUE)
pourcentage_rive_gauche <- (effectif_rive_gauche / total_anguilles) * 100

# Afficher les résultats
#pourcentage_rive_droite
#pourcentage_rive_gauche

#### résultats annuels


annee2$annee<-as.factor(annee2$annee)
annee2$ope_dic_identifiant<-as.factor(annee2$ope_dic_identifiant)

colnames(bio_ang)[colnames(bio_ang)=="car_valeur_quantitatif"] <- "taille"  #### pour renommer la colonne car_valeur_quantitatif en taille


######### récupération des pathologies des anguilles #########

colnames(patho)[colnames(patho)=="pco_lot_identifiant"] <- "lot_identifiant"

pat_lot<-merge(patho,lot,by="lot_identifiant",all.x = TRUE)
colnames(pat_lot)[colnames(pat_lot)=="lot_ope_identifiant"] <- "ope_identifiant"
pat_lot_ope<-  merge(pat_lot,ope,by="ope_identifiant",all.x = TRUE)
patho_constate<-select(pat_lot_ope,lot_identifiant,lot_tax_code,
                       lot_std_code,lot_effectif,pco_pat_code,pco_imp_code,pco_loc_code,
                       ope_identifiant,ope_date_debut,ope_date_fin,ope_dic_identifiant,ope_org_code)

patho_constate %>%
  group_by(pco_loc_code)%>%
  summarise(nb=n())


### Paramètres environneentaux 

##Dénomination station mesures enviro :
...
Coeff_maree<-subset(data_env,env_stm_identifiant==4)
Niveau_eau<-subset(data_env,env_stm_identifiant==8)
Temperature_air<-subset(data_env,env_stm_identifiant==7)
Temperature_eau<-subset(data_env,env_stm_identifiant==2)
Débit<-subset(data_env,env_stm_identifiant==1)



## Graphique des effectifs journaliers et cumulés 2024 

# Renommer les valeurs de DC
grdata <- grdata %>%
  mutate(DC = recode(DC, `102` = "Rive Gauche", `104` = "Rive Droite")) %>%
  arrange(fin_pas) %>%  # S'assurer que les données sont ordonnées par date
  group_by(DC) %>%      # Regrouper par rive (DC)
  mutate(effectif_cumule = cumsum(effectif_total)) %>%  # Calculer le cumul par rive
  ungroup()  # Ne pas oublier de dégrouper après l'opération

# Calculer le facteur d'échelle entre les deux jeux de données
max_effectif_total <- max(grdata$effectif_total)
max_effectif_cumule <- max(grdata$effectif_cumule)
scale_factor <- max_effectif_cumule / max_effectif_total

# Créer le graphique
graph <- ggplot(grdata, aes(x = fin_pas)) +
  geom_bar(aes(y = effectif_total, fill = DC), stat = "identity", position = "stack", alpha = 0.7) + # Barres empilées pour les effectifs journaliers
  geom_line(aes(y = effectif_cumule / scale_factor, color = DC, group = DC), size = 0.5) + # Lignes pour les effectifs cumulés
  geom_point(aes(y = effectif_cumule / scale_factor, color = DC, group = DC), size = 1) + # Points pour plus de clarté
  scale_y_continuous(
    name = "Effectifs journaliers",
    sec.axis = sec_axis(~ . * scale_factor, name = "Effectifs cumulés", 
                        labels = scales::comma)  # Utiliser scales::comma pour formater sans notation scientifique)  # Ajuster l'axe secondaire
  ) +
  scale_x_datetime(
    name = "Période de suivi", 
    date_labels = "%B",   # Afficher les mois en toutes lettres (en français avec Sys.setlocale)
    date_breaks = "1 month"  # Afficher les mois à chaque intervalle d'un mois
  ) +
  scale_fill_manual(name = "Effectifs journaliers", values = c("Rive Gauche" = "#1B4F72", "Rive Droite" = "#E74C3C")) + # Définir les couleurs des barres
  scale_color_manual(name = "Effectifs cumulés", values = c("Rive Gauche" = "#1B4F72", "Rive Droite" = "#E74C3C")) + # Définir les couleurs des lignes
  labs(
    title = "Effectifs journaliers et cumulés - 2024 ",
    x = "Période de suivi"
  ) +
  theme_bw() 
 

graph_oral <- graph + theme(
  plot.title = element_text(size = 20, face = "bold", hjust = 0.5),  # Centrer et mettre le titre en gras
  axis.text = element_text(size = 17),
  axis.title.y = element_text(size = 19),  # Taille du titre de l'axe des y
  axis.title.x = element_text(size = 19),  # Taille du titre de l'axe des x
  legend.text = element_text(size = 17),
  legend.title = element_text(size = 17)
)

png(file=str_c(imgwd,"graph_oral.png"),width=42,height=20,unit="cm",res=200)
print(graph_oral)
dev.off()

### Classe de taille des deux rives


colnames(bio_ang)[colnames(bio_ang)=="car_valeur_quantitatif"] <- "taille"  #### pour renommer la colonne car_valeur_quantitatif en taille

# Filtrer les données pour les opérations de la rive Gauche (102) et de la rive Droite (104)
gauche<-filter(bio_ang,annee==2024 & ope_dic_identifiant==102)
droite<-filter(bio_ang,annee==2024 & ope_dic_identifiant==104)



# Définir les limites des classes de taille pour la rive Gauche
breaks_gauche <- c(-Inf, 60, seq(65, 200, by = 5), Inf)

# Créer les classes de taille avec cut pour la rive Gauche
gauche$classes <- cut(gauche$taille, breaks = breaks_gauche, include.lowest = TRUE, right = FALSE)


# Définir les limites des classes de taille pour la rive Droite
breaks_droite <- c(-Inf, 60, seq(65, 200, by = 5), Inf)

# Créer les classes de taille avec cut pour la rive Droite
droite$classes <- cut(droite$taille, breaks = breaks_droite, include.lowest = TRUE, right = FALSE)

# Calculer les fréquences pour chaque classe de taille pour la rive Gauche
freq_gauche <- as.data.frame(table(gauche$classes))
colnames(freq_gauche) <- c("class", "frequency_gauche")

# Calculer les pourcentages pour la rive Gauche
freq_gauche <- freq_gauche %>%
  mutate(percentage_gauche = frequency_gauche / sum(frequency_gauche) * 100)

# Calculer les fréquences pour chaque classe de taille pour la rive Droite
freq_droite <- as.data.frame(table(droite$classes))
colnames(freq_droite) <- c("class", "frequency_droite")

# Calculer les pourcentages pour la rive Droite
freq_droite <- freq_droite %>%
  mutate(percentage_droite = frequency_droite / sum(frequency_droite) * 100)

# Fusionner les tables de fréquences des deux rives
freq_combined <- merge(freq_gauche, freq_droite, by = "class", all = TRUE)

# Remplacer les valeurs manquantes par 0 (si une classe n'est présente que dans une seule rive)
freq_combined[is.na(freq_combined)] <- 0

# Trier les classes de taille pour l'affichage correct dans le graphique
freq_combined$class <- factor(freq_combined$class, levels = unique(c(freq_gauche$class, freq_droite$class)))



# Créer une fonction pour transformer les labels des classes de taille
format_class_labels <- function(label) {
  label <- gsub("\\[", "", label)  # Supprimer les crochets ouvrants
  label <- gsub("\\)", "", label)  # Supprimer les parenthèses fermantes
  label <- gsub("\\(", "-", label) # Remplacer les parenthèses ouvrantes par des tirets
  label <- gsub(",", "-", label)   # Remplacer les virgules par des tirets
  label
}

# Créer l'histogramme avec ggplot2
classe_taille <- ggplot(freq_combined, aes(x = class)) +
  geom_bar(aes(y = percentage_gauche), stat = "identity", fill = "lightblue", color = "black", position = "stack") +
  geom_bar(aes(y = percentage_droite), stat = "identity", fill = "lightcoral", color = "black", position = "stack", width = 0.3) +
  labs(title = "Classes de Taille des individus par Rive",
       x = "Classe de Taille en mm",
       y = "Fréquence en %") +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +  # Convertir l'axe y en pourcentage
  scale_x_discrete(labels = format_class_labels) +  # Appliquer la fonction de formatage aux labels de l'axe x
  theme_bw() 

classe_taille_oral <- classe_taille + theme(
  plot.title = element_text(size = 20, face = "bold", hjust = 0.5),  # Centrer et mettre le titre en gras
...
png(file=str_c(imgwd,"classe_taille_oral.png"),width=42,height=20,unit="cm",res=200)
print(classe_taille_oral)
dev.off()


# Modifier les niveaux du facteur pour remplacer les numéros par les noms souhaités
annee2 <- annee2 %>%
  mutate(
    ope_dic_identifiant = factor(
      ope_dic_identifiant,
      levels = rev(levels(factor(ope_dic_identifiant))),
      labels = c("Rive droite", "Rive gauche")
    )
  )

# Calculer les totaux par année pour l'affichage au-dessus des barres
totals <- annee2 %>%
  group_by(annee) %>%
  summarize(total_effectif = round(sum(effectif), 0))

# Créer le graphique avec les modifications nécessaires
graph_annee<- ggplot(annee2) +
  aes(x = annee, y = effectif, fill = ope_dic_identifiant) +
  geom_col() +
  scale_fill_hue(direction = 1) +
  theme_minimal() +
  scale_y_continuous(name = "Effectif total", labels = scales::label_number()) +
  labs(
    title = "Bilan migratoire par année",
    fill = "Rive",  # Changement du titre de la légende
    x = "Années"
  ) +
  # Ajouter le total des effectifs au-dessus de chaque barre
  geom_text(data = totals, aes(x = annee, y = total_effectif, label = total_effectif),
            vjust = -0.5, size = 5, fontface = "plain", inherit.aes = FALSE) +
  guides(fill = guide_legend(reverse = TRUE)) 


graph_annee_oral <- graph_annee + theme(
  plot.title = element_text(size = 20, face = "bold", hjust = 0.5),  # Centrer et mettre le titre en gras
  axis.text = element_text(size = 17),
  axis.title.y = element_text(size = 19),  # Taille du titre de l'axe des y
  axis.title.x = element_text(size = 19),  # Taille du titre de l'axe des x
...
png(file=str_c(imgwd,"graph_annee_oral.png"),width=42,height=20,unit="cm",res=200)
print(graph_annee_oral)
dev.off()



## Morphologie taille des individus
bio_ang2024<-filter(bio_ang,annee==2024)

# Supposons que bio_ang est votre dataframe
bio_ang2024 <- bio_ang2024 %>%
  mutate(ope_dic_identifiant = recode(ope_dic_identifiant, `102` = "RG", `104` = "RD"))

# Calculer les valeurs extrêmes (outliers) pour chaque groupe
outliers <- bio_ang2024 %>%
  group_by(ope_dic_identifiant) %>%
  mutate(
  ...
  filter(taille < lower_bound | taille > upper_bound)

# Calculer le nombre d'observations pour chaque groupe
counts <- bio_ang2024 %>%
  group_by(ope_dic_identifiant) %>%
  summarise(n = n())

# Créer le graphique
taille<-ggplot(bio_ang2024, aes(x = ope_dic_identifiant, y = taille, fill = ope_dic_identifiant)) +
  geom_violin(trim = FALSE, color = "black", show.legend = FALSE) +  # Créer le violin plot sans légende
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA, show.legend = FALSE) +  # Ajouter un boxplot sans légende
  geom_point(data = outliers, aes(x = ope_dic_identifiant, y = taille), color = "black", size = 0.7, show.legend = FALSE) +  # Points noirs plus petits pour les valeurs extrêmes sans légende
  geom_text(data = counts, aes(x = ope_dic_identifiant, y = -Inf, label = paste("n =", n)), vjust = -0.5, size = 5, color = "black") +  # Ajouter les nombres d'observations sous chaque violon avec le texte "n ="
  geom_text(x = 1, y = max(bio_ang2024$taille) + 35, label = "a", size = 5) +  # Ajuster les coordonnées pour le groupe RG
  geom_text(x = 2, y = max(bio_ang2024$taille) + 35, label = "b", size = 5) +  # Ajuster les coordonnées pour le groupe RD
  labs(title = "Taille en mm des anguilles - 2024",
       x = "Rive",
       y = "Taille en mm") +
  scale_fill_manual(values = c("RG" = "lightblue", "RD" = "lightcoral")) +  # Définir les couleurs des violins
  scale_y_continuous(breaks = seq(0, 450, by = 50)) +  # Utiliser max(bio_ang2024$taille) pour fixer la limite de l'axe Y
  theme_minimal() 

taille_oral <- taille + theme(
  plot.title = element_text(size = 20, face = "bold", hjust = 0.5),  # Centrer et mettre le titre en gras
  axis.text = element_text(size = 17),
  axis.title.y = element_text(size = 19),  # Taille du titre de l'axe des y
  axis.title.x = element_text(size = 19),  # Taille du titre de l'axe des x
...
png(file=str_c(imgwd,"taille_oral.png"),width=42,height=20,unit="cm",res=200)
print(taille_oral)
dev.off()
...
##Graph avec les deux rives séparés
taille_mois<-ggplot(bio_ang2024, aes(x = factor(mois), y = taille, fill = ope_dic_identifiant)) +
  geom_boxplot() +
  labs(
    title = "Taille mensuelle des individus",
    x = "Mois",
    y = "Taille (mm)",
    fill = "Passes-pièges"
  ) +
  scale_fill_manual(
    values = c("RG" = "lightblue", "RD" = "lightcoral"),
    labels = c("RG" = "Rive gauche", "RD" = "Rive droite")
  ) +
  theme_minimal() 

taille_mois_oral <- taille_mois + theme(
  plot.title = element_text(size = 20, face = "bold", hjust = 0.5),  # Centrer et mettre le titre en gras
  axis.text = element_text(size = 17),
  axis.title.y = element_text(size = 19),  # Taille du titre de l'axe des y
  axis.title.x = element_text(size = 19),  # Taille du titre de l'axe des x
...
png(file=str_c(imgwd,"taille_mois_oral.png"),width=42,height=20,unit="cm",res=200)
print(taille_mois_oral)
dev.off()
...
# Calculer le facteur d'échelle entre les deux jeux de données
max_effectif_total <- max(grdata$effectif_total)
max_coeff <- max(Temperature_eau$env_valeur_quantitatif)
scale_factor <- max_coeff / max_effectif_total
...
# Créer le graphique
graph_temp_eau <- ggplot() +
  geom_bar(data=grdata, aes(x = fin_pas, y = effectif_total), stat = "identity", position = "stack", alpha = 0.7) + # Barres empilées pour les effectifs journaliers
  geom_line(data=Temperature_eau, aes(x= env_date_fin, y = env_valeur_quantitatif / scale_factor), size = 0.5, color = "darkblue") + # Lignes pour les coeff de marée
  scale_y_continuous(
    name = "Effectifs journaliers",
    sec.axis = sec_axis(~ . * scale_factor, name = " Température de la Seine en °C ", 
                        labels = scales::comma)  # Utiliser scales::comma pour formater sans notation scientifique)  # Ajuster l'axe secondaire
  ) +
  labs(
    title = "Effectifs journaliers par rapport au température de la Seine ",
    x = "Période de suivi"
  ) +
  scale_x_datetime(limits = c(min(grdata$fin_pas),max(grdata$fin_pas)),
                   date_labels = "%B", breaks = "month")

graph_temp_eau_oral <- graph_temp_eau + theme(
  plot.title = element_text(size = 20, face = "bold", hjust = 0.5),  # Centrer et mettre le titre en gras
  axis.text = element_text(size = 17),
  axis.title.y = element_text(size = 19),  # Taille du titre de l'axe des y
  axis.title.x = element_text(size = 19),  # Taille du titre de l'axe des x
...
png(file=str_c(imgwd,"graph_temp_eau_oral.png"),width=42,height=20,unit="cm",res=200)
print(graph_temp_eau_oral)
dev.off()
...
histo_debit$year<-strftime(histo_debit$env_date_debut,format="%Y")
histo_debit$mois<-strftime(histo_debit$env_date_debut,format="%m")

debit<-rename(histo_debit, bjo_jour=env_date_debut, bjo_annee=year,bjo_valeur=env_valeur_quantitatif,bjo_labelquantite=env_val_identifiant)
debit$bjo_labelquantite="Effectif_total"
dat<-fun_report_mig_interannual(debit,year = NULL,timesplit = NULL)

graph_debit<-ggplot() +
  geom_ribbon(data = dat, aes(ymin = mintab, ymax = maxtab, x = day, fill = "Amplitude des débits des années 2014 à 2024"), alpha = 0.5) +
  geom_line(data = dat, aes(x = day, y = mean, color = "Débit moyen")) +
  geom_line(data = subset(dat, year == 2024), aes(x = day, y = value, color = "Débit 2024")) +
  scale_x_datetime(name = "", 
                   limits = c(as.POSIXct("2000-04-01"), as.POSIXct("2000-09-30")), 
                   expand = c(0, 0),
                   date_labels = "%B",  # Mois en toutes lettres en français
                   date_breaks = "1 month") +  # Un mois entre chaque tick
  scale_y_continuous(name = "Débit") +  # Titre pour l'axe des Y
  scale_fill_manual(name = "Amplitude des débits des années 2014 à 2024", 
                    values = "#B9DDF1", 
                    guide = guide_legend(title = NULL)) +  # Définit le titre de la légende pour fill
  scale_color_manual(name = NULL,  # Légende sans titre pour les lignes
                     values = c("Débit moyen" = "#FEB7AA", "Débit 2024" = "#B5153A")) +
  ggtitle("Comparaison des débits 2024 et des débits moyens des années précédentes") +  # Titre du graphique
  theme_bw() 

graph_debit_oral <- graph_debit + theme(
  ...
  legend.text = element_text(size = 17),
  legend.title = element_text(size = 17),
  legend.position = "bottom",  # Placer la légende en dessous du graphique
  legend.box = "horizontal"    # Organiser les éléments de la légende horizontalement
)
png(file=str_c(imgwd,"graph_debit_oral.png"),width=42,height=20,unit="cm",res=200)
print(graph_debit_oral)
dev.off()

###Taille mois par année 


taille_ang_montaison_histo <- read.csv(str_c(datawd,"taille_ang_montaison_histo.csv"), sep = ";", header = TRUE)


taille_ang_montaison_histo$ope_dic_identifiant <- factor(taille_ang_montaison_histo$ope_dic_identifiant)
taille_ang_montaison_histo$annee <- factor(taille_ang_montaison_histo$annee)
taille_ang_montaison_histo$car_valeur_quantitatif <- as.numeric(taille_ang_montaison_histo$car_valeur_quantitatif)




taille_ang_montaison_histo <- taille_ang_montaison_histo %>%
  rename(taille = car_valeur_quantitatif)



    bio_ang_petit <- bio_ang %>%
      select(taille, annee, ope_dic_identifiant)  
    
fusion_data <- rbind(taille_ang_montaison_histo,bio_ang_petit)

taille_annee<-ggplot(fusion_data, aes(x = annee, y = taille, fill = ope_dic_identifiant)) +
  geom_boxplot() +
  labs(
    title = "Taille des individus par années",
    x = "Années",
    y = "Taille (mm)",
    fill = "Passes-pièges"
  ) +
  scale_fill_manual(
    values = c("102" = "lightblue", "104" = "lightcoral"),
    labels = c("102" = "Rive gauche", "104" = "Rive droite")
  ) +
  theme_minimal() 

taille_annee_oral <- taille_annee + theme(
  plot.title = element_text(size = 20, face = "bold", hjust = 0.5),  # Centrer et mettre le titre en gras
  axis.text = element_text(size = 17),
  axis.title.y = element_text(size = 19),  # Taille du titre de l'axe des y
  axis.title.x = element_text(size = 19),  # Taille du titre de l'axe des x
...
png(file=str_c(imgwd,"taille_annee_oral.png"),width=42,height=20,unit="cm",res=200)
print(taille_annee_oral)
dev.off()

###Flottang

flottang <- read.xlsx(str_c(datawd,"Données_Suivi_Flottang.xlsx"),sheet=1)

# Date cible
target_date <- as.Date("2020-07-03")

# Calcul de l'origine
origin_date <- target_date - 44015
print(origin_date)

flottang$Date_Fin <- as.Date(flottang$Date_Fin, origin = origin_date)

# Convertir Date en POSIXct avec l'heure à minuit
flottang$Date_Fin<- as.POSIXct(flottang$Date_Fin, format = "%Y-%m-%d", tz = "UTC")

# Supposons que votre colonne d'année s'appelle 'année'
flottang<- flottang %>% filter(Annee == 2024)

flottang$Date_Fin<- as.Date(flottang$Date_Fin, format="%Y-%m-%d")

flottang <- flottang %>% 
  filter(Date_Fin <= as.Date("2024-09-13"))

# Convertir Effectif en numérique
flottang$Effectif <- as.numeric(as.character(flottang$Effectif))

# Convertir Date en POSIXct avec l'heure à minuit
flottang$Date_Fin<- as.POSIXct(flottang$Date_Fin, format = "%Y-%m-%d", tz = "UTC")

# Utilisation de la fonction replace
flottang$Effectif <- replace(flottang$Effectif, is.na(flottang$Effectif), 0)

# Renommer les valeurs de DC
flottang <- flottang %>%
  mutate(Localisation = recode(Localisation, `AVR` = "Aval Rive droite", `AVT` = "Aval Turbine Rive Gauche")) %>%
  arrange(Date_Fin) %>%  # S'assurer que les données sont ordonnées par date
  group_by(Localisation) %>%      # Regrouper par rive (DC)
  mutate(Effectif_Cumule = cumsum(Effectif)) %>%  # Calculer le cumul par rive
  ungroup()  # Ne pas oublier de dégrouper après l'opération

# Calculer le facteur d'échelle entre les deux jeux de données
max_effectif_total <- max(flottang$Effectif)
max_effectif_cumule <- max(flottang$Effectif_Cumule)
scale_factor <- max_effectif_cumule / max_effectif_total

scale_factor <- 5

# Créer le graphique 
graph_flottang <- ggplot(flottang, aes(x = Date_Fin)) +
  geom_bar(aes(y = Effectif, fill = Localisation), stat = "identity", position = "stack", alpha = 0.7) +
  geom_line(aes(y = Effectif_Cumule / scale_factor, color = Localisation, group = Localisation), size = 0.5) +
  scale_y_continuous(
    name = "Effectifs journaliers",
    sec.axis = sec_axis(~ . * scale_factor, name = "Effectifs cumulés", 
                        labels = scales::comma)  # Formater sans notation scientifique
  ) +
  labs(
    title = "Effectifs journaliers et cumulés - Flottangs 2024",
    x = "Période de suivi",
    fill = "Effectifs journaliers ",  # Légende pour les barres
    color = "Effectifs cumulés"      # Légende pour les lignes
  ) +
  theme_bw() +
  scale_x_datetime(date_labels = "%B", breaks = "month") +
  scale_color_manual(values = c("red", "blue"), name = "Effectifs cumulés") +
  scale_fill_manual(values = c("red", "blue"), name = "Effectifs journaliers")

graph_flottang_oral <- graph_flottang + theme(
  plot.title = element_text(size = 20, face = "bold", hjust = 0.5),  # Centrer et mettre le titre en gras
  axis.text = element_text(size = 17),
  axis.title.y = element_text(size = 19),  # Taille du titre de l'axe des y
  axis.title.x = element_text(size = 19),  # Taille du titre de l'axe des x
...
png(file=str_c(imgwd,"graph_flottang_oral.png"),width=42,height=20,unit="cm",res=200)
print(graph_flottang_oral)
dev.off()
...
## Morphologie taille des individus flottang


# Vérifiez les valeurs uniques dans Taille_mm pour détecter les anomalies
unique(flottang$Taille_mm)

# Obtenez un résumé statistique pour voir les valeurs non numériques
summary(flottang$Taille_mm)

# Convertir la colonne Taille_mm en numérique
flottang$Taille_mm <- as.numeric(as.character(flottang$Taille_mm))

# Supprimer les lignes contenant des NA dans Taille_mm
flottang <- flottang[!is.na(flottang$Taille_mm), ]

# Filtrer les lignes où Taille_mm est différent de 0
flottang <- flottang[flottang$Taille_mm != 0, ]
...
taille_flottang<-ggplot(data = flottang, aes(x = Localisation, y = Taille_mm, fill = Localisation)) +
  geom_violin(trim = FALSE, alpha = 0.6) +
  geom_boxplot(width = 0.1, outlier.size = 1, color = "black", alpha = 0.5) +
  geom_text(data = counts, aes(x = Localisation, y = -Inf, label = paste("n =", n)), vjust = -0.3, size = 4, color = "black") +  # Ajouter les nombres d'observations sous chaque violon avec le texte "n ="
  geom_text(x = 1, y = max(flottang$Taille_mm) + 15, label = "a", size = 5) +  # Ajuster les coordonnées pour le groupe RG
  geom_text(x = 2, y = max(flottang$Taille_mm) + 15, label = "b", size = 5) +
  scale_fill_manual(values = c("Aval Turbine Rive Gauche" = "steelblue", "Aval Rive droite" = "salmon")) +
  labs(title = "Taille en mm des anguilles - Flottang 2024",
       x = "Flottang",
       y = "Taille en mm") +
  theme_minimal() 

taille_flottang_oral <- taille_flottang + theme(
  plot.title = element_text(size = 20, face = "bold", hjust = 0.5),  # Centrer et mettre le titre en gras
  axis.text = element_text(size = 17),
  axis.title.y = element_text(size = 19),  # Taille du titre de l'axe des y
  axis.title.x = element_text(size = 19),  # Taille du titre de l'axe des x
...
png(file=str_c(imgwd,"taille_flottang_oral.png"),width=42,height=20,unit="cm",res=200)
print(taille_flottang_oral)
dev.off()
...
data<-summarise(group_by(data_histo_RG,bjo_tax_code,bjo_std_code,
                         bjo_annee,bjo_jour,bjo_labelquantite,
                         bjo_org_code),bjo_valeur=sum(bjo_valeur)) %>%
  as.data.frame()

maxdate<-as.Date(max(data$bjo_jour))

day<-day(maxdate)
month<-month(maxdate)
maxdate<-as.Date(paste("2000",month,day,sep="-"),"%Y-%m-%d")


dat <- fun_report_mig_interannual(data)

dat$value[is.na(dat$value)]<-0 # sinon si il ne reste qu'une ligne peut planter
...
for (an in unique(dat$year)){
  ...
  dat[dat$year==an,"cumsum"] <- cumsum(dat[dat$year==an,"value"])
  dat[dat$year==an,"total_annuel"] <- max(dat[dat$year==an,"cumsum"])          
}
dat$day <- as.Date(dat$day)
dat$year <- as.factor(dat$year)
colnames(dat)[colnames(dat)=="total_annuel"] <- "total annuel"

dat1<-filter(dat,year==currentyear & day <=maxdate)
dat2<-filter(dat, year !=currentyear)

dat<-rbind(dat2,dat1)


g <- ggplot(dat)+geom_line(aes(x=day,y=cumsum,col=year),linewidth=1)+
  guides(color=guide_legend(title="Année")) 

g <- g+scale_y_continuous(name=gettext("Effectifs cumulés"),
                          labels = scales::comma_format(accuracy = 1, scale = 1, prefix = "", suffix = "",
                                                        big.mark = " ", decimal.mark = "."))
g <- g+scale_x_date(name=gettext("Période de suivi"),
                    date_breaks="months", 
                    date_labels = "%B",
                    limits=range(dat[dat$value>0 & dat$cumsum!=1,"day"]))+theme_bw()

g<-g+ggtitle("Effectifs cumulés interannuels - Rive gauche")
...
png(file=str_c(imgwd,"g_RG.png"),width=42,height=20,unit="cm",res=200)
print(g_RG)
dev.off()

### Rive droite 

data<-summarise(group_by(data_histo_RD,bjo_tax_code,bjo_std_code,
                         bjo_annee,bjo_jour,bjo_labelquantite,
                         bjo_org_code),bjo_valeur=sum(bjo_valeur)) %>%
  as.data.frame()
...
g<-g+ggtitle("Effectifs cumulés interannuels - Rive droite")
...
png(file=str_c(imgwd,"g_RD.png"),width=42,height=20,unit="cm",res=200)
print(g_RD)
dev.off()
...
Pour cette année 2024, le suivi a commencé le 2 avril, jour de la mise en route des pièges à anguilles. Les pièges de la rive droite aussi bien que la rive gauche ont fonctionné sans interruption pendant la période de suivi. Les premiers individus ont été observés le 8 avril en rive droite mais aussi en rive gauche. Les premiers individus de l’année avec une taille d’environ 80 mm ont été observés le 8 juillet. Pour cette année 2024, du 2 avril au 23 septembre un total de \Sexpr{round(sum(grdata$effectif_total))} anguilles a été comptabilisé soit un total de \Sexpr{total_rive_droite} (90\%) en rive droite et de \Sexpr{total_rive_gauche} (10\%) en rive gauche. La (Figure \ref{graph_oral}) représente les effectifs journaliers ainsi que les effectifs cumulés, deux pics sont visibles avec un premier début août et le principal mi-août que cela soit pour la rive droite ou la rive gauche. Cette année encore les anguilles ont majoritairement emprunté la rive droite par rapport à la rive gauche.
