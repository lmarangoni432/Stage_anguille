...
r_sample_char<-choice_c(r_sample_char,
                        ...
                        horodatedebut="2024-01-01",  ### rien avant car pas intégré dans la base
...
annee2<-r_ann@data
bio_ang<-report_sample_char@data
...
save(hauteur_eau,grdata,annee2,bio_ang,patho,lot,ope,data_env,file=str_c(datawd,"data_report.Rdata"))
...
nbangbio<-sum(bio_ang$lot_effectif)

bioangRG<-subset(bio_ang, bio_ang$ope_dic_identifiant==102)
bioangRD<-subset(bio_ang, bio_ang$ope_dic_identifiant==104)
...
gauche <- bio_ang[bio_ang$ope_dic_identifiant == 102, ]
droite <- bio_ang[bio_ang$ope_dic_identifiant == 104, ]
...
# Créer le graphique
taille<-ggplot(bio_ang, aes(x = ope_dic_identifiant, y = taille, fill = ope_dic_identifiant)) +
  ...
  geom_text(x = 1, y = max(bio_ang$taille) + 35, label = "a", size = 5) +  # Ajuster les coordonnées pour le groupe RG
  geom_text(x = 2, y = max(bio_ang$taille) + 35, label = "b", size = 5) +  # Ajuster les coordonnées pour le groupe RD
  ...
  scale_y_continuous(breaks = seq(0, 450, by = 50)) +  # Utiliser max(bio_ang$taille) pour fixer la limite de l'axe Y
  theme_minimal() 
...
taille_mois<-ggplot(bio_ang, aes(x = factor(mois), y = taille, fill = ope_dic_identifiant)) +
  geom_boxplot() +
  labs(
...
Débit$year<-strftime(Débit$env_date_debut,format="%Y")
Débit$mois<-strftime(Débit$env_date_debut,format="%m")

debit<-rename(Débit, bjo_jour=env_date_debut, bjo_annee=year,bjo_valeur=env_valeur_quantitatif,bjo_labelquantite=env_val_identifiant)
...
fusion_data <- full_join(taille_ang_montaison_histo, bio_ang, 
                             by = c("taille", "ope_dic_identifiant", "annee"))


taille_annee<-ggplot(fusion_data, aes(x = factor(annee), y = taille, fill = ope_dic_identifiant)) +
  geom_boxplot() +
  labs(
